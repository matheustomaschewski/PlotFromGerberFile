...
imagemscan = cv2.imread("plotscan.jpg", 0)
# cv2.imshow("ImagemEscaneada", imagemscan)
...
for k in range(len(lerlinhas)): # Faz leitura linha a linha
    ...
    if 'X' == linha[0]: # Verifica se o primeiro caracter da String é 'X'. Se for, é uma coordenada.
        ...
        if op == "01":
            fig1 = plt.figure(1)
            plt.axis('Equal')
            plt.plot([xold, x],[yold, y], color=cor, lw = linew*pol, solid_joinstyle=jstyle, solid_capstyle=cstyle)
        else:
            if op =="02":
                plt.figure(2)
                plt.axis('Equal')
                ...
                if op == "03":
                    ...
                    if shapetype == 'C':
                        #fancybox = mpatches.FancyBboxPatch((x-(linewx/2),y-(linewy/2)),linewx,linewy,color='orange') 
...
fig1.savefig('img01-teste2.png')

# 1 pixel (X) = 0,264583333333334 millimeter [mm]